...
3. Run exact-hash `workstation-reconcile --apply`. Marketplace/Plugin mutation participates in the source-cutover rollback
   ...
   both versioned Skills, and performs exact readback before releasing rollback state. Validate Bootstrap `2.2.1`, Scout `5.8.0`,
...
Enter only from a refreshed new task that actually loaded Bootstrap 2.2.1 after an `inspect` result requested reload. Use the Codex
