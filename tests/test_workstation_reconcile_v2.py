...
class WorkstationReconcileV2Tests(unittest.TestCase):
    ...
    def exact_host(self) -> dict[str, object]:
        return {
            ...
            "doctor": "verified",
            "bootstrap_skill": {
                "status": "unchanged", "version": "2.2.1", "content_sha256": "c" * 64,
            },
            "scout_skill": {
                "status": "unchanged", "version": "5.8.0", "content_sha256": "d" * 64,
            },
        }
    ...
    def test_consumer_scope_distinguishes_exact_bytes_from_same_version_drift(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual("2.2.1", drifted["projects"][0]["skills"][0]["version"])
    ...
    def test_desired_bundle_comes_from_verified_release_and_portable_bytes(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual("2.2.1", desired["bootstrap_version"])
    ...
    def test_host_observer_reads_live_doctor_and_physical_skills(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            for name, version in (
                ("agent-memory-workstation-bootstrap", "2.2.1"),
                ("global-owner-scout", "5.8.0"),
            ):
            ...
            self.assertEqual("2.2.1", observed["bootstrap_skill"]["version"])
